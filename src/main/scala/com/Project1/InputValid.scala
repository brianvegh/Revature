...
object InputValid {
  ...
  private val INT_MAX = 2147483647 //default int value of highest possible int value + 1
  ...
  private def getString(prompt: String, illegalCharacters: String="", alphaOnly: Boolean=false, alphaNumericOnly:Boolean=false,
                        blankOK:Boolean=false,minimumLength:Int=0,maximumLength:Int=INT_MAX, unavailableStrings:Array[String]=null) = {
    ...
    do {
      ...
      try {
        ...
        if (alphaNumericOnly) for (c <- result.toCharArray) {
          if ((c < 65 || c > 90) && (c < 97 || c > 122) &&(c<48||c>57) && (c != ' ') ) throw new Exception("Input is invalid. Only letters and numbers are valid.")
        }
        if (minimumLength > 0) {
          if (result.length<minimumLength) throw new Exception(s"Input must be $minimumLength characters long.")
        }
        if (maximumLength < INT_MAX) {
          if (result.length>maximumLength) throw new Exception(s"Input must be $maximumLength characters long.")
        }
        if (unavailableStrings!=null){
          (unavailableStrings.foreach((x:String) =>if (result==x) {
            throw new Exception(s"Sorry, the username $result is unavailable")
              }
            )
          )
        }
  ...
  /**
   * returns password string from user containing between $minimumLength and $maximumLength characters
   *
   * @param prompt prompt displayed to user
   * @param minimumLength int minimum string length
   * @param maximumLength int maximum string length
   * @return returns string from user
   */
  def getUniqueUsername(prompt: String, unavailableUsernames:Array[String]): String = getString(prompt, alphaNumericOnly=true, unavailableStrings=unavailableUsernames)
   ...
   * @return returns string from user
   */
  def getPassword(prompt: String, minimumLength:Int=5, maximumLength:Int=20): String = getString(prompt, minimumLength=5,maximumLength = 20)

  /**
   * returns username string from user containing alphanumeric characters between $minimumLength and $maximumLength
   *
   * @param prompt prompt displayed to user
   * @param minimumLength int minimum string length
   * @param maximumLength int maximum string length
   * @return returns string from user
   */
  def getUsername(prompt: String, minimumLength:Int, maximumLength:Int): String = getString(prompt, minimumLength=5,maximumLength = 20,alphaNumericOnly = true)
